$package('rca.weixpaylistOriginal');
rca.weixpaylistOriginal = function(){
	var _box = null;
	var _this = {
		config:{
  			dataGrid:{
	...
	$('#orderWeixPayList').dialog({
		buttons : [ {
			...
			handler : function() {
				...
				$.messager.confirm('确认', '确认重新获取'+statdate+'微信账单？', function(r) {
					if (r) {
						$.messager.progress({
				            title:'请稍后',
				            msg:'正在获取微信账单...'
